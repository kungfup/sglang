...
class VITSchedulerClient:
    ...
    def submit_async(
        ...
        try:
            ...
            logger.info(f"[VIT Client] 📋 pending_requests keys: {list(self.pending_requests.keys())}")
            logger.info(f"[VIT Client] 📋 pending_requests id: {id(self.pending_requests)}")

            # 立即验证 pending_requests
            if request_id in self.pending_requests:
                logger.info(f"[VIT Client] ✅ Verified: {request_id} is in pending_requests")
            else:
    ...
    def poll_results(self) -> Dict[str, torch.Tensor]:
        ...
        if out:
            ...
            # 低频诊断，避免刷屏
            if self._poll_count <= 20 or (len(self.pending_requests) > 0 and self._poll_count % 100 == 0):
