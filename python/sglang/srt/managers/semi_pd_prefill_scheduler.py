...
class SemiPDPrefillScheduler(SemiPDScheduler):
    ...
    def get_next_batch_to_run(self) -> Optional[ScheduleBatch]:
        ...
        # Add debugging for request reception
        if self.attn_tp_rank == 0:
            logger.info(f"[PREFILL] recv_from_tokenizer is: {self.recv_from_tokenizer}")
            if hasattr(self, 'recv_from_tokenizer') and self.recv_from_tokenizer is not None:
                logger.info(f"[PREFILL] recv_from_tokenizer type: {type(self.recv_from_tokenizer)}")
        ...
        if self.waiting_queue and self.attn_tp_rank == 0:
            ...
            resp = None  # Temporary placeholder
