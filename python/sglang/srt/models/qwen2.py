...
class Qwen2ForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        if self.pp_group.world_size > 1 and config.tie_word_embeddings:
            # 🔧 修复：只有DECODE进程才执行分布式通信
            # 检查当前进程是否为DECODE进程
            is_decode_process = os.getenv('SGLANG_ENABLE_SEMI_PD', 'False') == 'True' and \
                               os.getenv('SGLANG_INSTANCE_ROLE', 'DECODE') == 'DECODE'
            
            if is_decode_process:
                logger.info(f"🔧 [MODEL_LOADER] DECODE进程：执行PP权重绑定通信")
                if self.pp_group.is_first_rank:
                    self.pp_group.send(
                ...
                # 修复日志信息：区分DECODE和PREFILL进程
                instance_role = os.getenv('SGLANG_INSTANCE_ROLE', 'UNKNOWN')
                if instance_role == 'PREFILL':
                    logger.info(f"🔧 [MODEL_LOADER] PREFILL进程：跳过PP权重绑定通信（避免Meta tensor错误）")
                else:
